...
> 
> - [English](README.md) | [Español](README.es.md) | **中文** (当前) | [日本語](README.ja.md) | [Português](README.pt-BR.md) | [한국어](README.ko.md) | [Deutsch](README.de.md) | [Italiano](README.it.md) | [Français](README.fr.md)
> - **文档**: [English](docs/en/) | [Español](docs/es/) | **中文** (docs/zh-CN/) | [日本語](docs/ja/) | [Português](docs/pt-BR/) | [한국어](docs/ko/) | [Deutsch](docs/de/) | [Italiano](docs/it/) | [Français](docs/fr/)
...
**⚠️ 注意**: 这会创建真实的 AWS 资源（总计约 $0.17）。完成后记得运行清理脚本！

## 适合谁使用

**非常适合：** 刚接触 AWS IoT Core 的云开发者、解决方案架构师和 DevOps 工程师

**你需要：** 基本的 AWS 知识、一些 Python 基础，以及熟悉命令行操作

**学习级别：** 通过动手实践的助理级别方法

## 🔧 使用 AWS SDK 构建

这个项目使用官方 AWS SDK，为你提供真实的 AWS IoT Core 体验：

### **Boto3 - AWS SDK for Python**
- **作用**: 支持所有 AWS IoT Registry 操作、证书管理和 Rules Engine 交互
- **版本**: `>=1.26.0`
...
- **作用**: 使用 X.509 证书实现与 AWS IoT Core 的真实 MQTT 通信
- **版本**: `>=1.11.0`
...
**为什么这些 SDK 很重要：**
- **生产就绪**: 与真实 IoT 应用程序中使用的 SDK 相同
- **安全性**: 内置支持 AWS IoT 安全最佳实践
- **可靠性**: AWS 官方维护的库，具有完善的错误处理
- **学习价值**: 你将体验真实的 AWS IoT 开发模式
...
**⚠️ 重要安全提示**: 请使用专门的开发或学习 AWS 账户。不要在包含生产 IoT 资源的账户中运行这些脚本。虽然清理脚本有多重安全功能，但使用隔离的环境进行学习始终是最佳实践。

### 费用说明

**这个项目会创建真实的 AWS 资源，总费用约 $0.17。**
...
export AWS_IOT_LANG=pt-BR  # 葡萄牙语
export AWS_IOT_LANG=ko     # 韩语
export AWS_IOT_LANG=de     # 德语
export AWS_IOT_LANG=it     # 意大利语
export AWS_IOT_LANG=fr     # 法语
...
- **葡萄牙语** (`pt-BR`, `portuguese`, `português`, `pt`) - 完整翻译可用
- **韩语** (`ko`, `korean`, `한국어`, `kr`) - 完整翻译可用
- **德语** (`de`, `german`, `deutsch`) - 完整翻译可用
- **意大利语** (`it`, `italian`, `italiano`) - 完整翻译可用
- **法语** (`fr`, `french`, `français`) - 完整翻译可用

## 🌍 多语言支持

所有学习脚本都支持英语、西班牙语、日语、中文、葡萄牙语、韩语、德语、意大利语和法语。你选择的语言会影响：
...
**作用**: 创建真实的 IoT 资源用于动手学习
**创建内容**: 20 个 Things、3 个 Thing Types、4 个 Thing Groups

### 2. 🔍 探索 IoT Registry API
**文件**: `scripts/iot_registry_explorer.py`
**作用**: 学习 AWS IoT Registry API 的交互式工具
**功能**: 8 个核心 API，包含详细说明和真实 API 调用

### 3. 🔐 管理证书和策略
**文件**: `scripts/certificate_manager.py`
**作用**: 通过证书和策略管理学习 AWS IoT 安全
**功能**: 证书创建、策略附加、外部证书注册
...
**作用**: 使用 MQTT 协议体验实时 IoT 通信
**功能**: 交互式命令行界面、主题订阅、消息发布

### 5. 🌟 探索 AWS IoT Device Shadow
**文件**: `scripts/device_shadow_explorer.py`
**作用**: 使用 AWS IoT Device Shadow 学习设备状态同步
**功能**: 交互式影子管理、状态更新、增量处理

### 6. ⚙️ 探索 IoT Rules Engine
**文件**: `scripts/iot_rules_explorer.py`
**作用**: 使用 IoT Rules Engine 学习消息路由和处理
...
**⚠️ 重要**: 学习完成后记得运行清理，避免持续产生 AWS 费用。
...
- **解决方案**: 
  - 检查你在设置期间是否使用了自定义前缀
  - 使用正确的前缀和 `--things-prefix`
  - 在 AWS 控制台中验证资源是否存在

**问题："权限被拒绝"错误**
- **原因**: AWS 凭证缺少必要的 IoT 权限
- **解决方案**: 确保你的 IAM 用户/角色具有 IoT 完全访问权限

**问题："依赖冲突"错误**
- **原因**: 资源具有未处理的依赖关系
- **解决方案**: 脚本应该会自动处理这个问题。如果持续存在，使用 `--debug` 运行查看详细信息
...
1. **总是先试试 dry-run**: 在执行之前预览将被删除的内容
